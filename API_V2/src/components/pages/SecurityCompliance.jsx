...
const SecurityCompliance = ({ subview = "overview" }) => {
  ...
  const {
    riskData,
    accountsData,
    complianceData: contextComplianceData,
  } = dataContext;

  // Utilisation des données du contexte ou des mocks si non disponibles
  const complianceData =
    contextComplianceData?.length > 0
      ? contextComplianceData
      : mockComplianceData;
  const risksData = riskData?.length > 0 ? riskData : mockRiskData;

  // Vérification des données requises
  const hasRequiredData = accountsData?.length > 0 || complianceData;
  ...
  try {
    ...
    console.error("Erreur dans le composant SecurityCompliance:", error);
    return (
...
SecurityCompliance.propTypes = {
  subview: PropTypes.oneOf(["overview", "compliance", "violations", "risks"]),
};
