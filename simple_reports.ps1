...
# Version avec authentification unique
...
if (!(Get-Module -ListAvailable -Name PSPAS)) {
    try {
        ...
        Install-Module PSPAS -Scope CurrentUser -Force -Repository PSGallery
    } catch {
...
try {
    ...
    if ($null -eq $session) {
        throw "Session nulle retournée"
    }
    Write-Host "Authentification réussie!" -ForegroundColor Green
    
    # Stockage du jeton dans une variable pour les scripts qui l'utilisent directement
    $token = $session.sessionToken
    Write-Host "Jeton d'authentification obtenu" -ForegroundColor Green
} 
catch {
